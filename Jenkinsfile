      ...
      steps {
        script {
          ...
          withEnv(["MVN_HOME=${tool name: 'Maven 3', type: 'maven'}", "JAVA_HOME=${tool name: 'JDK17', type: 'jdk'}"]) {
            ...
            sh "curl -s -L -o resources/jre-windows.zip https://github.com/adoptium/temurin21-binaries/releases/download/jdk-21.0.12%2B8/OpenJDK21U-jre_x64_windows_hotspot_21.0.12_8.zip"
            sh "curl -s -L -o resources/jre-linux.tar.gz https://github.com/adoptium/temurin21-binaries/releases/download/jdk-21.0.12%2B8/OpenJDK21U-jre_x64_linux_hotspot_21.0.12_8.tar.gz"
          }
        }
      }
    }
      ...
      steps {
        script {
          ...
          if ("develop" == env.BRANCH_NAME) {
            ...
            sh 'unzip -q resources/jre-windows.zip && mv jdk-21.0.12+8-jre jre'
            sh 'npx @vscode/vsce package --pre-release --target win32-x64'
            sh 'rm -rf jre/ && tar xfz resources/jre-linux.tar.gz && mv jdk-21.0.12+8-jre jre'

...
export function runGUI(filename: string, project: OpenEdgeProjectConfig) {
  ...
  const prmFileName = path.join(
    tmpdir(),
    'rungui-' + crypto.randomBytes(16).toString('hex') + '.json',
  );
  const cfgFile = {
    verbose: false,
  ...
  cp.spawn(currProfile.getExecutable(true), prms2, {
    env: env,
    cwd: project.rootDir,
    detached: true,
  });
...
export function openInAB(
  ...
  const prmFileName = path.join(
    tmpdir(),
    'openInAB-' + crypto.randomBytes(16).toString('hex') + '.json',
  );
  const cfgFile = {
    verbose: false,
  ...
  cp.spawn(project.getExecutable(true), prms2, {
    env: env,
    cwd: rootDir,
    detached: true,
  });
...
export function openInProcEditor(
  filename: string,
  rootDir: string,
  project: ProfileConfig,
) {
  ...
  const prmFileName = path.join(
    tmpdir(),
    'openInProcEd-' + crypto.randomBytes(16).toString('hex') + '.json',
  );
  const cfgFile = {
    verbose: false,
    databases: project.dbConnections,
